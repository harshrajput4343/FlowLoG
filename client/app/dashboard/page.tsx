...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Paint boards from localStorage immediately
    try {
  ...
  const handleUseTemplate = async () => {
    ...
    if (!token || token === 'guest-token') { router.push('/login'); return; }
  ...
  return (
    <div className={styles.layoutContainer}>
            ...
            <section className={styles.section}>
              ...
              <div className={styles.boardGrid}>
                {recentBoards.slice(0, 4).map(board => (
                  <Link key={board.id} href={`/b/${board.id}`} className={styles.boardCard} style={getBackgroundStyle(board.background)}>
            ...
            <div className={styles.templateGrid}>
              {HOME_TEMPLATES.map(template => (
                <div
                  ...
                  onClick={() => { setSelectedTemplate(template); setTemplateBoardTitle(template.title); }}
                >
            ...
            <div className={styles.boardGrid}>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!token || token === 'guest-token') { router.push('/login'); }
      ...
      {showCreateModal && (
        <CreateBoardModal onClose={() => setShowCreateModal(false)} onCreated={handleBoardCreated} />
      )}
    </div>
  );
}
